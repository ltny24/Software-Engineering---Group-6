...
export default function ClassTransferPage() {
  ...
  const handleCancelSection = async () => {
    ...
    if (
      !window.confirm('Cancel this section? It will no longer accept enrollments or transfers.')
    ) {
      return;
    }
    try {
      setCancelling(true);
      setError(null);
      await adminClassControlService.cancelSection(sourceId);
      toast.success('Section cancelled.');
    } catch (err: any) {
      setError(err.response?.data?.message || 'Failed to cancel the section.');
  ...
  const needsJustification = overrideCapacity || overrideConflict;
        ...
        <>
          <div className="transfer-panel">
            ...
            <div className="source-summary">
              ...
              {roster.offering.status === 'Cancelled' && (
            ...
            <div className="action-row">
              <button
                ...
                disabled={cancelling || roster.offering.status === 'Cancelled'}
              >
                {cancelling ? 'Cancelling…' : 'Cancel this section'}
              </button>
            </div>
            ...
            <textarea
              ...
              placeholder={
                needsJustification
                  ? 'Justification (required for overrides, recorded in the audit log)…'
